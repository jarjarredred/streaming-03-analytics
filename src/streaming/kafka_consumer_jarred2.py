"""src/streaming/kafka_consumer_jarred2.py.
...
OUTPUT_CSV: Final[Path] = OUTPUT_DIR / "consumed_sales_jarred2.csv"
...
# Modified initialize_output to track regional sales via tuple.
def initialize_output() -> tuple[RunningStats, dict[str, float]]:
    ...
    # Return a tuple: (Standard Stats, New Regional Dictionary)
    return RunningStats(), {}
...
def process_message(
    ...
    *,
    region_lookup: dict[str, float],
    stats: RunningStats,
    regional_totals: dict[str, float], # New argument
) -> dict[str, Any] | None:
    ...
    # --- SMALL MODIFICATION START ---
    LOG.info(f"Processed Sale #{stats.count}")  # Added count tracking
    LOG.info(f"subtotal={enriched['subtotal']}")
    LOG.info(f"tax={enriched['tax_amount']}")
    LOG.info(f"total={enriched['total']}")
    LOG.info(f"running_total={stats.total:.2f}")
    # --- SMALL MODIFICATION END ---
    region = enriched.get("region_id", "Unknown")
    current_region_total = regional_totals.get(region, 0.0)
    regional_totals[region] = current_region_total + enriched["total"]

    LOG.info(f"Updated {region} total: ${regional_totals[region]:.2f}")
...
def main() -> None:
    ...
    stats, regional_totals = initialize_output()
